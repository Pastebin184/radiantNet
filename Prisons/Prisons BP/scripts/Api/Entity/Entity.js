...
export class Entity {
     ...
     * @param {string} command Command to run
     * @returns {Promise<boolean>} Whethor or not there was an error
     */
    async runCommandAsync(command) {
        return new Promise((e) => this.entity.runCommandAsync(command).then(a => e(true), a => e(false)));
    }
    /**
...
export class Player extends Entity {
     ...
     */
    getGamemode() {
        if ([...world.getPlayers({ gameMode: GameMode.survival })].map(e => e.id).includes(this.entity.id))
            return "survival";
        if ([...world.getPlayers({ gameMode: GameMode.creative })].map(e => e.id).includes(this.entity.id))
            return "creative";
        if ([...world.getPlayers({ gameMode: GameMode.adventure })].map(e => e.id).includes(this.entity.id))
            return "adventure";
        if ([...world.getPlayers({ gameMode: GameMode.spectator })].map(e => e.id).includes(this.entity.id))
            return "spectator";
        return "unknown";
    }
    /**
