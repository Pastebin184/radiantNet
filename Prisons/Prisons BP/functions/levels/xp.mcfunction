execute as @a[hasitem={item=coal}] run scoreboard players add @s money 1
execute as @a[hasitem={item=coal}] run scoreboard players add @s xpSell 1
execute as @a[hasitem={item=coal}] run give @s re_coal 1 0
execute as @a run clear @a[hasitem={item=coal}] coal 0 1
execute as @a[hasitem={item=raw_copper}] run scoreboard players add @s money 1
execute as @a[hasitem={item=raw_copper}] run scoreboard players add @s xpSell 1
execute as @a[hasitem={item=raw_copper}] run give @s re_copper_ingot 1 0
execute as @a run clear @a[hasitem={item=raw_copper}] raw_copper 0 1
execute as @a[hasitem={item=raw_iron}] run scoreboard players add @s money 3
execute as @a[hasitem={item=raw_iron}] run scoreboard players add @s xpSell 8
execute as @a[hasitem={item=raw_iron}] run give @s re_iron_ingot 1 0
execute as @a run clear @a[hasitem={item=raw_iron}] raw_iron 0 1
execute as @a[hasitem={item=raw_gold}] run scoreboard players add @s money 4
execute as @a[hasitem={item=raw_gold}] run scoreboard players add @s xpSell 16
execute as @a[hasitem={item=raw_gold}] run give @s re_gold_ingot 1 0
execute as @a run clear @a[hasitem={item=raw_gold}] raw_gold 0 1
execute as @a[hasitem={item=lapis_lazuli}] run scoreboard players add @s money 4
execute as @a[hasitem={item=lapis_lazuli}] run scoreboard players add @s xpSell 2
execute as @a[hasitem={item=lapis_lazuli}] run give @s re_lapis_lazuli 1 0
execute as @a run clear @a[hasitem={item=lapis_lazuli}] lapis_lazuli 0 1
execute as @a[hasitem={item=redstone}] run scoreboard players add @s money 1
execute as @a[hasitem={item=redstone}] run scoreboard players add @s xpSell 12
execute as @a[hasitem={item=redstone}] run give @s re_redstone 1 0
execute as @a run clear @a[hasitem={item=redstone}] redstone 0 1
execute as @a[hasitem={item=diamond}] run scoreboard players add @s money 7
execute as @a[hasitem={item=diamond}] run scoreboard players add @s xpSell 35
execute as @a[hasitem={item=diamond}] run give @s re_diamond 1 0
execute as @a run clear @a[hasitem={item=diamond}] diamond 0 1
execute as @a[hasitem={item=netherrack}] run scoreboard players add @s money 8
execute as @a[hasitem={item=netherrack}] run scoreboard players add @s xpSell 40
execute as @a[hasitem={item=netherrack}] run give @s re_netherrack 1 0
execute as @a run clear @a[hasitem={item=netherrack}] netherrack 0 1
execute as @a[hasitem={item=quartz}] run scoreboard players add @s money 9
execute as @a[hasitem={item=quartz}] run scoreboard players add @s xpSell 40
execute as @a[hasitem={item=quartz}] run give @s re_quartz 1 0
execute as @a run clear @a[hasitem={item=quartz}] quartz 0 1
execute as @a[hasitem={item=ancient_debris}] run scoreboard players add @s money 10
execute as @a[hasitem={item=ancient_debris}] run scoreboard players add @s xpSell 300
execute as @a[hasitem={item=ancient_debris}] run give @s re_ancient_debris 1 0
execute as @a run clear @a[hasitem={item=ancient_debris}] ancient_debris 0 1
execute as @a[hasitem={item=emerald}] run scoreboard players add @s money 11
execute as @a[hasitem={item=emerald}] run scoreboard players add @s xpSell 55
execute as @a[hasitem={item=emerald}] run give @s re_emerald 1 0
execute as @a run clear @a[hasitem={item=emerald}] emerald 0 1
execute as @a[hasitem={item=end_stone}] run scoreboard players add @s money 12
execute as @a[hasitem={item=end_stone}] run scoreboard players add @s xpSell 132
execute as @a[hasitem={item=end_stone}] run give @s re_end_stone 1 0
execute as @a run clear @a[hasitem={item=end_stone}] end_stone 0 1
execute as @a[hasitem={item=obsidian}] run scoreboard players add @s money 13
execute as @a[hasitem={item=obsidian}] run scoreboard players add @s xpSell 220
execute as @a[hasitem={item=obsidian}] run give @s re_obsidian 1 0
execute as @a run clear @a[hasitem={item=obsidian}] obsidian 0 1
execute as @a[hasitem={item=iron_block}] run scoreboard players add @s money 14
execute as @a[hasitem={item=iron_block}] run scoreboard players add @s xpSell 159
execute as @a run clear @a[hasitem={item=iron_block}] iron_block 0 1
execute as @a[hasitem={item=gold_block}] run scoreboard players add @s money 15
execute as @a[hasitem={item=gold_block}] run scoreboard players add @s xpSell 200
execute as @a run clear @a[hasitem={item=gold_block}] gold_block 0 1
execute as @a[hasitem={item=lapis_block}] run scoreboard players add @s money 16
execute as @a[hasitem={item=lapis_block}] run scoreboard players add @s xpSell 232
execute as @a run clear @a[hasitem={item=lapis_block}] lapis_block 0 1
execute as @a[hasitem={item=redstone_block}] run scoreboard players add @s money 17
execute as @a[hasitem={item=redstone_block}] run scoreboard players add @s xpSell 250
execute as @a run clear @a[hasitem={item=redstone_block}] redstone_block 0 1
execute as @a[hasitem={item=diamond_block}] run scoreboard players add @s money 18
execute as @a[hasitem={item=diamond_block}] run scoreboard players add @s xpSell 300
execute as @a run clear @a[hasitem={item=diamond_block}] diamond_block 0 1
execute as @a[hasitem={item=emerald_block}] run scoreboard players add @s money 19
execute as @a[hasitem={item=emerald_block}] run scoreboard players add @s xpSell 350
execute as @a run clear @a[hasitem={item=emerald_block}] emerald_block 0 1
execute as @a[hasitem={item=netherite_block}] run scoreboard players add @s money 20
execute as @a[hasitem={item=netherite_block}] run scoreboard players add @s xpSell 375
execute as @a run clear @a[hasitem={item=netherite_block}] netherite_block 0 1
execute as @a[hasitem={item=lodestone}] run scoreboard players add @s beaconsell 1
execute as @a[hasitem={item=lodestone}] run scoreboard players add @s xpSell 5
execute as @a run clear @a[hasitem={item=lodestone}] lodestone 0 1
